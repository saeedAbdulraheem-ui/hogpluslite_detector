...
contrast_val=0
sharpness_val=0
vs = VideoStream(src=0,framerate=10, usePiCamera=True, contrast=contrast_val, brightness=55, drc_strength='high').start()
...
while True:
	...
	for encoding in encodings:
		...
		if True in matches:
			...
			# we set 0.5 as a strict cutoff for facial detection. Any matches with
			#euclidean distance above 0.5 will get a value of 1
			face_distances = face_recognition.face_distance(data["encodings"], encoding)
			fd_mean = statistics.mean([fd if (fd<0.5) else 1 for fd in face_distances])
			proba[name] = (1 - fd_mean) * 100 # probability as a percentage
	...
	for ((top, right, bottom, left), name) in zip(boxes, names):
		...
		text = name
		if(name != 'Unknown'):
			text = "{}: {:.2f}%".format(name,proba[name])
